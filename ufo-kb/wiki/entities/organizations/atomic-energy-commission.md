---
type: entity
subtype: organization
name: Atomic Energy Commission
aliases: [AEC]
type_field: gov-agency
parent_org: US Federal Government
country: USA
active_period: "1946–1974"
tags: [aec, nuclear, doe, egng, mcmahon-secrecy-act, classification, uap-secrecy]
sources: []
summary: "US civilian agency established by the Atomic Energy Act of 1946 (McMahon Act) to control nuclear weapons development; prime contractor was EG&G; succeeded in 1974 by the Department of Energy; the classification regime it established is the legal hook David Grusch and Bob Maguire identify as governing UAP material secrecy."
---

# Atomic Energy Commission (AEC)

US civilian agency created by the [[mcmahon-secrecy-act]] (Atomic Energy Act of 1946) to control the development, production, and use of atomic energy and nuclear weapons outside of military control. It was dissolved in 1974 and its functions split between the [[department-of-energy]] and the Nuclear Regulatory Commission.

## Classification Regime

The AEC's classification framework — established by the McMahon Act — is the legal structure that [[david-grusch]] and [[bob-maguire]] independently identify as the statute that *automatically classifies* all recovered UAP material. The mechanism: if recovered technology is classified as involving atomic energy or its applications, the AEC (now DOE) classification framework places it beyond normal congressional oversight.

[[edward-condon]] — who later led the [[condon-report]] UFO study — co-authored the McMahon Act.

## EG&G as Prime Contractor

[[egng]] (EG&G) was the AEC's prime contractor, running the Nevada Test Site and providing classified technical services across AEC facilities. This is the root of EG&G's later presence at [[area-51]] — the AEC-to-DOE lineage made EG&G the natural operational arm for the most sensitive black programs.

## Successor

The AEC was succeeded by the [[department-of-energy]], which inherited its classification regime and, according to disclosure research, its custodianship over UAP-related recovered materials.

## Connections

- Mentioned in [[1f16VvXaSSE]]
- [[mcmahon-secrecy-act]] — founding legislation; co-authored by Condon
- [[egng]] — prime contractor; ran Nevada Test Site
- [[department-of-energy]] — successor agency; inherited classification regime
- [[edward-condon]] — co-authored the founding statute; later ran UFO debunking study
- [[david-grusch]] — cites AEC/DOE classification framework as UAP secrecy mechanism
- [[bob-maguire]] — independently identifies same statute
- [[doe-secret-science-nexus]] — concept connecting AEC legacy to UAP programs
- [[atomic-energy-act-classification]] — the operative classification framework
