---
type: entity
summary: "Handwritten notes allegedly taken by Dr. Eric Davis from a conversation with Admiral Thomas R. Wilson in a Las Vegas parking lot on October 16, 2002. Wilson allegedly confirmed knowledge of a private-sector crash-retrieval and reverse-engineering program that he was denied access to as DIA Director. One of the most significant alleged documents in UAP disclosure history."
sources: [raw/youtube-transcripts/BEWz4SXfyCQ.md]
title: Wilson-Davis Memo
authors: [eric-davis]
date: "2002-10-16 (alleged)"
type: memo
classification: unclassified (leaked)
tags: [crash-retrieval, reverse-engineering, disclosure, wilson, eric-davis, special-access-programs]
---

# Wilson-Davis Memo

Fifteen pages of handwritten notes allegedly taken by physicist [[eric-davis]] immediately after a private conversation with Vice Admiral Thomas R. Wilson at a parking lot in Las Vegas on October 16, 2002. Wilson had recently retired as Director of the Defense Intelligence Agency (DIA).

## Alleged Contents

According to the notes:
- Wilson confirmed that in 1997, as J-2 (Director of Intelligence on the Joint Chiefs), he attempted to gain access to a Special Access Program involving crashed UAP hardware and reverse engineering.
- He was denied access by program gatekeepers from a private defense contractor, who told him the program existed but that he had no "need to know."
- The contractor's lawyers threatened consequences if Wilson pursued the matter.
- Wilson confirmed that "they" (the program managers) had crash-retrieved hardware and biologics.

## Provenance

The document appeared online in June 2019, leaked from the estate of astronaut [[edgar-mitchell]]. Both Wilson and Davis have publicly denied or declined to confirm the document's authenticity. The document's internal details have been extensively analyzed by UAP researchers and journalists.

## Connections

- Mentioned in [[3yrKVu35uSE]]

- Mentioned in [[2NVDCtSxIac]]
- [[eric-davis]] — alleged author; physicist at EarthTech / BIAC; Hal Puthoff colleague
- [[john-von-neumann]] — mentioned in related UAP briefing networks
- [[hal-puthoff]] — Davis's professional network; advanced propulsion research
- [[reverse-engineered-craft]] — the primary subject of the alleged conversation (crash-retrieved hardware and reverse engineering)
- [[disclosure-narratives]] — the Wilson-Davis memo as a key document in controlled-disclosure debate
- [[david-grusch]] — Grusch's allegations in 2023 corroborate the broader pattern Wilson described
- [[source-type-frameworks]] — classified as leaked/alleged; provenance disputed
- [[jeremy-corbell]] — authenticated on camera in [[BEWz4SXfyCQ]]: "I'm in a position to know and it is a real document"; confirmed EG&G meeting location and private-industry SAP denial
- [[egng]] — the meeting location per Corbell's authentication: EG&G Special Projects, Las Vegas
- [[BEWz4SXfyCQ]] — source where Corbell authenticates it on camera (JRE #1315, 2019)
